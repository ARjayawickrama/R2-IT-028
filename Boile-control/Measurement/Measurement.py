...
app = FastAPI(title="FishGoPRO Measurement & Gateway API")
...
def on_connect(client, userdata, flags, rc):
    print(f"[✓] Connected to HiveMQ Broker (Code: {rc})")
...
async def startup_event():
    ...
    print("[✓] FastAPI Event Loop Initialized on Port 8001")

# =====================================================
# REST ENDPOINTS (MONGODB CRUD)
# =====================================================
...
async def delete_measurement(item_id: str):
    try:
        res = await measurements_collection.delete_one({"_id": ObjectId(item_id)})
...
async def clear_all_measurements():
...
async def reset_zero():
    ...
    if main_loop and main_loop.is_running():
        asyncio.run_coroutine_threadsafe(
            broadcast_ws({"type": "data", "data": latest_data}), main_loop
        )
    return {"success": True}
