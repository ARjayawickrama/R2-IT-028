...
const BACKEND_IP = "http://localhost:8001";
...
export default function Measurement() {
    ...
    const connectWebSocket = () => {
      try {
        ...
        ws.onopen = () => {
          if (!isUnmounted) {
            setConnState("ok");
            setStatusLine("ESP32 Online & Gateway Connected (Port 8001)");
          }
        };
        ...
        if (!isUnmounted) reconnectTimeout = setTimeout(connectWebSocket, 3000);
      }
    };
  ...
  const handleStartScan = () => {
    setLeftCm(0.0); setCenterCm(0.0); setRightCm(0.0); setPeakCm(0.0);
    sendCommand("/api/scan/start", "START_SCAN");
  };

  const handleZeroReset = () => {
    setLeftCm(0.0); setCenterCm(0.0); setRightCm(0.0); setPeakCm(0.0); setWeight(0.0);
        ...
        <div>
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            <span className="px-2.5 py-0.5 rounded-md bg-teal-50 text-teal-700 border border-teal-200 text-xs font-mono font-bold">
              DATABASE & HARDWARE GATEWAY (PORT 8001)
            </span>
          </div>
            ...
            <div className="p-5 sm:p-6 border-b border-slate-200 flex items-center justify-between bg-slate-50">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Saved records of fish thickness and weight readings.
                </p>
              </div>
                  ...
                  {filteredReports.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-12 text-center text-slate-400">
                        No records found in database. Trigger "Start Laser Profile Scan" to capture readings.
                      </td>
                    </tr>
                  ) : (
